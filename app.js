...
form.addEventListener("submit", function (e) {
  ...
  //空白，小于等于0和大于9的情况都只返回一个随机段落
  if (isNaN(value) || value <= 0 || value > 9) {
    result.innerHTML = `<p class="reslut">${text[random]}</p>`;
  } else {
    let tempText = text.slice(0, value);
    //让每个item都自成一段
    //注意map是会返回new array的，需要用join()将array的内容转化为string。
    tempText = tempText
      .map(function (item) {
    ...
    // 直接写的话，段落是不分段的，会堆在一起
    // result.innerHTML = `<p class="reslut">${tempText}</p>`;
  }
});
